...
namespace MyForm
{
	...
	public ref class Rectangl : Element
	{
	protected:
		int width;
		int height;
	...
	public:
		Rectangl(Color color, Point p1, Point p2, float penWidth, DashStyle style)
		{
			pen = gcnew Pen(color, penWidth);
			pen->DashStyle = style;
			this->color = color;
	...
	public:
		...
		void Add(Point p)
		{
			...
			boundRect.Inflate(-penWidth, -penWidth);         // Reduce the rectangle by the pen width
			if (p.X < boundRect.X)
			{
			...
			boundRect.Inflate(penWidth, penWidth);          // Inflate the rectangle by the pen width
		}
